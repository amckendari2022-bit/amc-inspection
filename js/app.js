...
function getOnDutyInline(group) {
var list = onDutyState[group];
if (!list || list.length === 0) return "-";
return list.join(", ");
}
...
doc.setFont("helvetica", "bold");
doc.setFontSize(8);
doc.text("PERSONIL ON DUTY", margin, y);
y += 5;
doc.setFont("helvetica", "normal");

var groups = [
{ label: "Office Hour", key: "officeHour" },
{ label: "Dinas Pagi", key: "pagi" },
{ label: "Dinas Siang", key: "siang" }
];
for (var gIdx = 0; gIdx < groups.length; gIdx++) {
var g = groups[gIdx];
var names = onDutyState[g.key];
var namesText = names && names.length > 0 ? names.join(", ") : "-";
doc.setFont("helvetica", "bold");
doc.text(g.label + ":", margin + 2, y);
doc.setFont("helvetica", "normal");
var splitNames = doc.splitTextToSize(namesText, pageWidth - margin * 2 - 35);
doc.text(splitNames, margin + 32, y);
y += Math.max(5, splitNames.length * 4);
}
y += 4;
...
var pdfLine = window._lastPdfUrl ? ("\nLink Checklist:\n" + window._lastPdfUrl + "\n") : "";
