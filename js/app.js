...
doc.setFontSize(8);
doc.setTextColor(0, 0, 0);
doc.text("PERSONIL ON DUTY", pageWidth / 2, y + 5, { align: "center" });
y += 12;
...
// Kejadian Khusus
if (y > 260) { doc.addPage(); y = 15; }
var kejadian = document.getElementById("specialEvents") ? document.getElementById("specialEvents").value || "-" : "-";
doc.setFont("helvetica", "bold");
doc.setFontSize(9);
doc.text("Kejadian Khusus:", margin + 3, y);
y += 7;
doc.setFont("helvetica", "normal");
var splitKejadian = doc.splitTextToSize(kejadian, pageWidth - margin * 2 - 6);
...
y += 7;
doc.setFont("helvetica", "normal");
var splitKesimpulan = doc.splitTextToSize(kesimpulan, pageWidth - margin * 2 - 6);
...
doc.setFont("helvetica", "normal");
doc.setFontSize(8);
doc.text(parkingLink.substring(0, 90), margin + 3, y);
y += 8;
}
...
y += 6;
if (sigData) {
try { doc.addImage(sigData, "PNG", margin + 115, y, 45, 22); } catch (e) {}
}
y += 25;
doc.text("(________________)", margin + 12, y);
doc.text("(" + officer + ")", margin + 125, y);
